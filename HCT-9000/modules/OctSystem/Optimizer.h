...
namespace OctSystem
{
	...
	class OCTSYSTEM_DLL_API Optimizer
	{
	public:
		Optimizer();
		virtual ~Optimizer();

		static bool initOptimizer(OctDevice::MainBoard* board, Scanner* scanner, RetinaAlign* retAlign);
		static bool initOptimizer(OctDevice::MainBoard * board, RetinaAlign * retAlign);
		static bool isInitiated(void);
		static void clearAutoMeasureResult(void);

		static bool startAutoOptimize(bool noFundus, bool dioptFocus=false, bool topogrphyMode=false);
		static bool startAutoPosition(void);
		static bool startAutoMeasure(bool macular, bool cornea);
		
		static bool startAutoCorneaFocus(bool compenLens);
		static bool startAutoRetinaFocus(bool compenLens);
		static bool startAutoLensFrontFocus(int corneaReferPos, float corneaDioptPos);
		static bool startAutoLensBackFocus(int frontReferPos, float frontDioptPos);

		static bool startAutoAxialLength(int measureCount = 10, bool keepOnError = false);
		static bool startAutoLensThickness(int measureCount = 10, bool keepOnError = false);
		static bool startAutoLensThickness2(int measureCount = 10, bool keepOnError = false, bool referFixed = false);

		static bool startAutoDiopterFocus(AutoFocusCompleteCallback* callback = nullptr);
		static bool startAutoPolarization(AutoPolarizationCompleteCallback* callback=nullptr);
		static bool startAutoReference(AutoReferenceCompleteCallback* callback=nullptr, bool captureCornea=false, 
										bool narrowTarget=false, bool smallMoveStep=false, bool autoCenter=false, 
										bool measureMode=false, bool upperTarget=false);
		static bool startAutoSplitFocus(void);

		static void cancelAutoReference(bool wait);
		static void cancelAutoDiopterFocus(bool wait);
		static void cancelAutoPolarization(bool wait);
		static void cancelAutoSplitFocus(bool wait);
		static void cancelAutoMeasure(bool wait);

		static bool isAutoDiopterFocusing(void);
		static bool isAutoPolarizing(void);
		static bool isAutoReferencing(void);
		static bool isAutoSplitFocusing(void);
		...
		static bool isMeasuringRetina(void);
		static bool isMeasuringCornea(void);

	protected:
		struct OptimizerImpl;
		static std::unique_ptr<OptimizerImpl> d_ptr;
		static OptimizerImpl& getImpl(void);
		...
		static void threadAutoMeasureFunction(void);

		static void threadAutoCorneaFocusFunction(void);
		static void threadAutoRetinaFocusFunction(void);
		static void threadAutoLensFrontFocusFunction(void);
		static void threadAutoLensBackFocusFunction(void);
		static void threadAutoAxialLengthFunction(void);
		static void threadAutoLensThicknessFunction(void);
		static void threadAutoLensThicknessFunction2(void);
