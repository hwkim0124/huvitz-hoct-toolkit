...
// the situation. 
NormData::NormDiscNFL::NormDiscNFL(NormDiscNFL && rhs) = default;
NormDiscNFL & NormData::NormDiscNFL::operator=(NormDiscNFL && rhs) = default;


void NormData::NormDiscNFL::insertData(Ethinicity race, Gender gender, int sector, float inclin,
...
void NormData::NormDiscNFL::insertData(Ethinicity race, Gender gender, int sector, float inclin, float inter, float stddev)
{
	auto& dataMap = getDataMap(race, gender);
...
int NormData::NormDiscNFL::getPercentile(Ethinicity race, Gender gender, int age, EyeSide side, int sector, float value)
{
	auto& dataMap = getDataMap(race, gender);
	if (dataMap.empty()) {
		return 0;
...
int NormData::NormDiscNFL::getPercentile2(Ethinicity race, Gender gender, int age, EyeSide side, int sector, float value)
{
	auto& dataMap = getDataMap(race, gender);
	if (dataMap.empty()) {
		return 0;
...
float NormData::NormDiscNFL::getNormThickness(Ethinicity race, Gender gender, int age, EyeSide side, int sector, int percentile)
{
	auto& dataMap = getDataMap(race, gender);
	if (dataMap.empty()) {
		return 0;
...
CppUtil::CvImage NormData::NormDiscNFL::makeImage(Ethinicity race, Gender gender, int age, EyeSide side, const std::vector<float>& data, int lines, int points, int width, int height, bool isWide) 
{
...
std::map<int, DiscNFL_Data>& NormData::NormDiscNFL::getDataMap(Ethinicity race, Gender gender)
{
...
NormDiscNFL::NormDiscNFLImpl & NormData::NormDiscNFL::getImpl(void) const
{
	return *d_ptr;
}
