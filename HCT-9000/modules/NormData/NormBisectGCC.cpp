...
// the situation. 
NormData::NormBisectGCC::NormBisectGCC(NormBisectGCC && rhs) = default;
NormBisectGCC & NormData::NormBisectGCC::operator=(NormBisectGCC && rhs) = default;


void NormData::NormBisectGCC::insertData(Ethinicity race, Gender gender, const char * sector, float inclin,
...
void NormData::NormBisectGCC::insertData(Ethinicity race, Gender gender, const char * sector, float inclin, float inter, float stddev)
{
	auto& dataMap = getDataMap(race, gender);
...
int NormData::NormBisectGCC::getPercentile(Ethinicity race, Gender gender, int age, EyeSide side, int sector, float value)
{
auto& dataMap = getDataMap(race, gender);
	if (dataMap.empty()) {
		return 0;
...
const char* NormData::NormBisectGCC::getSectorStr(EyeSide side, int sector)
{
	switch (sector) {
	case 0:
		return "SUP";
...
std::map<std::string, BisectGCC_Data>& NormData::NormBisectGCC::getDataMap(Ethinicity race, Gender gender)
{
...
NormBisectGCC::NormBisectGCCImpl & NormData::NormBisectGCC::getImpl(void) const
{
	return *d_ptr;
}
