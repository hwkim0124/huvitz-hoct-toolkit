...
namespace NormData
{
	...
	public:
		...
		explicit ClockNFL_Data(float inclin_, float* inters_) {
			this->inclin = inclin_;
			for (int i = 0; i < N_VALUES; i++) {
		...
		explicit ClockNFL_Data(float inclin_, float bias_, float stddev_) {
			this->inclin = inclin_;
			this->bias = bias_;
			this->stddev = stddev_;
		}
	...
	class NORMDATA_DLL_API NormClockNFL
	{
	public:
		NormClockNFL();
		virtual ~NormClockNFL();

		NormClockNFL(NormClockNFL&& rhs);
		NormClockNFL& operator=(NormClockNFL&& rhs);

		// Prevent copy construction and assignment. 
		NormClockNFL(const NormClockNFL& rhs) = delete;
		NormClockNFL& operator=(const NormClockNFL& rhs) = delete;
	...
	private:
		...
		int calculateWeightedRegionPercentile(int sector, const std::vector<float>& tsnitPercentiles,
			bool isCompareType, EyeSide realEyeSide, bool isMacularDisc, int dataIndex);
		std::vector<int> getClockTsnitIndices(int regionIndex, bool needsEyeSideConversion);
		static std::vector<float> s_clockTsnitData;
		static std::vector<float> s_clockTsnitData95;
		static std::vector<float> s_clockTsnitData5;
		static std::vector<float> s_clockTsnitData1;

	protected:
		std::map<std::string, ClockNFL_Data>& getDataMap(Ethinicity race, Gender gender);

	private:
		struct NormClockNFLImpl;
		std::unique_ptr<NormClockNFLImpl> d_ptr;
		NormClockNFLImpl& getImpl(void) const;
	};
}
