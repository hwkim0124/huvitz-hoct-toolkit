...
namespace NormData
{
	...
	public:
		...
		explicit DiscNFL_Data(float inclin_, float* inters_) {
			this->inclin = inclin_;
			for (int i = 0; i < N_VALUES; i++) {
		...
		explicit DiscNFL_Data(float inclin_, float bias_, float stddev_) {
			this->inclin = inclin_;
			this->bias = bias_;
			this->stddev = stddev_;
		}
	...
	class NORMDATA_DLL_API NormDiscNFL : NormDeviationMap
	{
	public:
		NormDiscNFL();
		virtual ~NormDiscNFL();

		NormDiscNFL(NormDiscNFL&& rhs);
		NormDiscNFL& operator=(NormDiscNFL&& rhs);

		// Prevent copy construction and assignment. 
		NormDiscNFL(const NormDiscNFL& rhs) = delete;
		NormDiscNFL& operator=(const NormDiscNFL& rhs) = delete;
	...
	protected:
		std::map<int, DiscNFL_Data>& getDataMap(Ethinicity race, Gender gender);

	private:
		struct NormDiscNFLImpl;
		std::unique_ptr<NormDiscNFLImpl> d_ptr;
		NormDiscNFLImpl& getImpl(void) const;
	};
}
