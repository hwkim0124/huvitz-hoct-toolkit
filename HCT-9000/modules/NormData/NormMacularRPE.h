...
namespace NormData
{
	...
	public:
		...
		explicit MacularRPE_Data(float inclin_, float* inters_) {
			this->inclin = inclin_;
			for (int i = 0; i < N_VALUES; i++) {
		...
		explicit MacularRPE_Data(float inclin_, float bias_, float stddev_) {
			this->inclin = inclin_;
			this->bias = bias_;
			this->stddev = stddev_;
		}
	...
	class NORMDATA_DLL_API NormMacularRPE : NormDeviationMap
	{
	public:
		NormMacularRPE();
		virtual ~NormMacularRPE();

		NormMacularRPE(NormMacularRPE&& rhs);
		NormMacularRPE& operator=(NormMacularRPE&& rhs);

		// Prevent copy construction and assignment. 
		NormMacularRPE(const NormMacularRPE& rhs) = delete;
		NormMacularRPE& operator=(const NormMacularRPE& rhs) = delete;
	...
	protected:
		std::map<int, MacularRPE_Data>& getDataMap(Ethinicity race, Gender gender);

	private:
		struct NormMacularRPEImpl;
		std::unique_ptr<NormMacularRPEImpl> d_ptr;
		NormMacularRPEImpl& getImpl(void) const;
	};
}
