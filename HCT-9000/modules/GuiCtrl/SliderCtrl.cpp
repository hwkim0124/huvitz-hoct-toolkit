...
void SliderCtrl::OnPaint() 
{
	...
	GuiCtrl::CMemDC dc( &dcOrigin, &m_rect, m_bTransparentChannel );
	...
	if (!m_bEnable) {
		DrawBitmap(&dc, 0, 0, m_nWidth, m_nHeight, &dcMem, 0, 0, &m_bmChannelActiveMask, m_bTransparentChannel);
	}
...
//
void SliderCtrl::SetRangeMax(int nMax, BOOL bRedraw)
{
	m_nMax = nMax;
	if( bRedraw )
		Invalidate();
...
//
void SliderCtrl::SetRangeMin(int nMin, BOOL bRedraw)
{
	m_nMin = nMin;
	if( bRedraw )
		Invalidate();
...
//
void SliderCtrl::SetRange(int nMin, int nMax, BOOL bRedraw)
{
...
//
void SliderCtrl::SetPosition(int nPos)
{
	m_nPos = nPos;
...
//
int SliderCtrl::SetPageSize(int nSize)
{
	int nRet = m_nPage;
...
//
void SliderCtrl::SetMargin(int nLeft, int nTop, int nRight, int nBottom )
{
	SetMarginLeft( nLeft );
	SetMarginTop( nTop );
...
//
void SliderCtrl::Enable(BOOL bEnable)
{
	m_bEnable = bEnable;
...
//
void SliderCtrl::DrawFocusRect(BOOL bDraw, BOOL bRedraw)
{
...
//
BOOL SliderCtrl::SetBitmapChannel(
...
//			Function failes to load bitmaps.
//
BOOL SliderCtrl::SetBitmapThumb(
...
//
int SliderCtrl::Pos2Pixel(int nPos)
{
	if( m_bVertical ) {
...
//
int SliderCtrl::Pixel2Pos(int nPixel)
{
	if( m_bVertical ) {
...
//
void SliderCtrl::CopyBackground(
...
//
void SliderCtrl::RestoreBackground(
...
// By Raja Segar
//
void SliderCtrl::DrawTransparentBitmap(
	CDC *pDC, int xStart, int yStart, int wWidth, int wHeight,
...
//
void SliderCtrl::OnKillFocus(CWnd* pNewWnd) 
{
...
//
void SliderCtrl::OnSetFocus(CWnd* pOldWnd) 
{
