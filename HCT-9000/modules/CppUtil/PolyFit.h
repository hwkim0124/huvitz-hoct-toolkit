...
template<typename T>
inline std::vector<T> polyfit(const std::vector<T>& oX, const std::vector<T>& oY, int nDegree)
{
...
template<typename T>
inline std::vector<T> polyval(const std::vector<T>& oCoeff, const std::vector<T>& oX)
{
	size_t nCount = oX.size();
