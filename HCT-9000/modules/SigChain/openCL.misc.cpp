...
void createOpenCLProgram(cl_program & program, cl_context context, cl_device_id deviceId, const char * source)
{
	//std::string pstr = source;
	//size_t plen = pstr.length();

	// program = ::clCreateProgramWithSource(context, 1, (const char**)&pstr, NULL, NULL);
	program = ::clCreateProgramWithSource(context, 1, &source, NULL, NULL);
	if (program == NULL) {
		throw std::runtime_error("Program with source can't be created");
	}
...
void releaseOpenCLProgram(cl_program & program)
{
	if (program != NULL) {
		OPENCL_V_THROW(::clReleaseProgram(program),
...
void createOpenCLKernel(cl_program program, cl_kernel & kernel, const char * kernel_name)
{
	kernel = ::clCreateKernel(program, kernel_name, NULL);
	if (kernel == NULL) {
		throw std::runtime_error("Kernel can't be created");
	}
	return;
}
...
{
	if (kernel != NULL) {
		OPENCL_V_THROW(::clReleaseKernel(kernel),
			"Releasing Kernel( ::clReleaseKernel() )");
		kernel = NULL;
	}
	return;
}
