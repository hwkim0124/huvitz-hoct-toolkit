...
bool SemtRetina::BoundaryONL::designPathConstraints(void)
{
	...
	for (int x = 0; x < width; x++) {
		if (ilms[x] <= 0) {
			...
			for (int y = y2; y >= yc; y--) {
				auto idx = y * width + x;
				if (indice[idx] <= CLASS_RPE || indice[idx] == CLASS_DISC_HEAD) {
					break;
				}
				else {
					y2 = y;
	...
	if (band->isNerveHeadRangeValid() && band->isNerveHeadDiscCupShaped()) {
		...
		moves = crta->getPathDiscRangeDeltaONL();
		for (int x = x1; x <= x2; ++x) {
			delt[x] = moves;
		}
	}

	const int WINDOW_SIZE1 = crta->getPathSmoothWindowONL();
	const int WINDOW_SIZE2 = crta->getPathSmoothWindowONL();
	const int DEGREE = 1;
	auto upp2 = CppUtil::SgFilter::smoothInts(upps, WINDOW_SIZE1, DEGREE);
	auto low2 = CppUtil::SgFilter::smoothInts(lows, WINDOW_SIZE2, DEGREE);
...
bool SemtRetina::BoundaryONL::preparePathCostMap(void)
{
	...
	*/
	...
	Mat matRest = Mat::zeros(height, width, CV_32F);
	float* p_rest = matRest.ptr<float>(0);
	for (int i = 0; i < N; ++i) {
		float val1 = max(vits[i], nfls[i]);
		float val2 = max(rpes[i], chrs[i]);
		p_rest[i] = max(val1, val2) * -0.5f;
	}
	...
	cv::add(matCost, matRest, matCost);
	// cv::multiply(matProb, matInvs, matCost);
